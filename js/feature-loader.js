...
(function (global) {
  ...
  var PORTAL_ASSET_V = "research-desktop-paint-20260825b";
  ...
  async function ensureMarketing() {
    ...
    state.marketing = (async function () {
      ...
      await injectPartial("creator-video-clipper-modal.html");
      loadCss(asset("creator-video-clipper-modal.css"));
      ...
      await loadScriptsSequentialOptional([
        ...
        asset("creator-video-clipper-modal.js"),
      ]);
    })();
  ...
  async function ensureResearch() {
    if (state.research) {
      try { await state.research; } catch (_e) { /* retry below */ }
